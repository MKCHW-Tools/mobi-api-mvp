...
router.get('/tokens/refresh', validateToken, async (req, res) => {
    ...
    const updated = User.update(_id, {token: newAccessToken, refreshToken: newRefreshToken})
...
router.post('/tokens/in-validate-token', auth, authInvalidateTokens, async (req, res) => {
    ...
    tokens.forEach(token => {
        const {token :{oldToken}, type} = token
        const owner = (type == 1) ? User.findOne({accesToken: oldToken}) : User.findOne({refreshToken: oldToken})
        if(owner._id) {
            (type == 1) ? User.update(owner._id,{accessToken:''}) : User.update(owner._id,{refreshToken:''})
        }
    })
